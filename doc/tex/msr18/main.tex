...
\multicolumn{3}{c}{\rowcolor{gray}\textcolor{white}{\textbf{Problems in Search-based Software Engineering}}} \\ 
\multicolumn{3}{c}{
		...
		\begin{minipage}[b]{0.63\linewidth}
		        \textbf{Problem: } SBSE converts a SE problem into an optimization problem, where the goal is to find the maxima or minima of objective functions \llm{$y_i=f_i(x)$, $1 \leq i \leq m$, where  $f_i$ are the objective / evaluation functions, $m$ is the number of objectives,} x is called the \textit{independent variable}, and $y_i$ are the \textit{dependent variables}.\\
		        %\textbf{Problem Types: } The problems can be either single-~\cite{nair2017faster} or multi-~\cite{chen2017sampling} or many-objective~\cite{mkaouer2014high}.\TODO{We are not telling what the difference between multi- and many- is here. If we need to save space for the figure to fit in one page, we could possibly omit ``many''.}\\
		        \textbf{Global Maximum/Minimum: } For single objective ($m=1$) problems, algorithms aim at finding a single solution able to optimize the objective function, i.e., a global maximum/minimum. \\%there is one global maxima or minima \TODO{This statement is wrong. Single objective problems can have multiple global optima! The difference is that they have only one function to be optimized. I think what you mean here is that, in single objective problems, we are interested in retrieving a single solution able to maximize or minimize the objective function.}.\\
		        ...
		        \textbf{Actual \llm{Pareto Front} (PF): } A list of best solutions of a space is called Actual PF ($a\in A$). As this can be unknowable in practice or prohibitively expensive to generate, it is common to take from the union of all optimization outcomes all non-dominated solutions and use it as the (approximated) PF.\\ %This requires knowledge of the whole space---which is not available in real-world setting.\\
		        \textbf{Predicted PF: } The solutions found by \llm{an optimization} algorithm are called the Predicted PF ($p\in P$).
				%  Problems tacked in SBSE can be described as finding the best (minimum or maximum) of a function ($y=f(x)$), where x is called the \textit{independent variable}, $f$ is the evaluation function, and $y$ the \textit{dependent variable}. $y$ can either be single objective or multi-objective. In a multi-objective setting, there no single `best' solution instead there is number of `best' solution. This can also be stated as solutions which are better (or \textit{dominates}) all other solution---which is not considered `best' solutions, and no better than other `best' solution as defined by domination criterion. The domination criterion  can be defined as: ``A solution $x_1$ is said to dominate
    %             another solution $x_2$, if $x_1$ is no worse than $x_2$ in all objectives
    %             and $x_1$ is strictly better than $x_2$ in at least one objective.'' The list of these `best' solutions are called the \textit{Pareto Front}. The figure represents a solution space of a (synthetic) multi-objective problem with two objectives: $\mathit{Objective}_1$ and $\mathit{Objective}_2$. This best point can be called as the \textit{Utopia Point} and the worst is called the \textit{Nadir Point}. The points $A_i$ represents the \textit{Actual Pareto Front} ($A$). The goal of the multi-objective optimizer is to find set of solutions which are closest to the actual Pareto frontier. The solutions found by the optimizers are called the \textit{Predicted Pareto Front} ($P$). \vspace{0.1cm}
		\end{minipage}
		
} \\ 
\multicolumn{3}{c}{\rowcolor{gray}\textcolor{white}{\textbf{Components of Meta-heuristic Algorithms (search-based optimization algorithms such as NSGA-II~\cite{deb2000fast}, SPEA2~\cite{zitzler2001spea2}, MOEA/D~\cite{zhang2007moea}, AGE~\cite{wagner2015age})}}} \\
\multicolumn{3}{c}{
...
\noindent\textbf{Representation: } \llm{This defines how solutions $x$ are represented internally by the algorithm.
...
\noindent\textbf{Population: } A set of solutions maintained by the algorithm using the representation.

\noindent\textbf{Initialization: } The process of search \llm{typically} starts by creating random solutions (valid or invalid)~\cite{saber2017seeding, chen2017beyond, chen2017sampling, henard2015combining}.
...
% 				\includegraphics[height=4.7cm]{doc/tex/msr18/img/sbse_code.png}
% 				\vspace{1cm}
% \end{minipage}
}
 \\
\multicolumn{3}{c}{\rowcolor{gray}\textcolor{white}{\textbf{Performance Measures (Refer to \cite{wang2016practical,chand2015manyemo} for more detail)}}} \\ 
...
% to many of the models in tiny.cc/sbse to 
...
\multirow{-4}{*}{MSR} & \multirow{-3}{*}{\cellcolor[HTML]{EFEFEF}Text Classification} & \multirow{-3}{*}{\cellcolor[HTML]{EFEFEF}Text} & \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor[HTML]{EFEFEF}-}} & \cellcolor[HTML]{EFEFEF}1 & \cellcolor[HTML]{EFEFEF}StackOverflow & \cellcolor[HTML]{EFEFEF}\censor{\href{http://tiny.cc/SOProcess}{SOProcess}} & \multirow{-3}{*}{\cellcolor[HTML]{EFEFEF}\cite{agrawalwrong}} \\ \midrule
 ...
 & Text Discovery & Text & - & 4 & Reading Faster & \censor{\href{http://tiny.cc/raise_data_fastread}{raise\_data\_fastread}} & \cite{yu2016read} \\
...
known'') behavior of the method. Please refer to  \cite{nair2018finding} and  \cite{nair2017using,nair2017faster} to similar strategies. The reproduction package is available in \censor{\url{http://tiny.cc/flashrepo/}}.
